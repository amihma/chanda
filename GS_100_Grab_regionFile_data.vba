// ========================================
// REGION DATA COLLECTION - 100 VERSION
// CONFIGURATION
// ========================================
...
function refreshRegionData() {
  try {
    ...
    const rootFolder = parentFolder.getParents().next();  // 100_Project folder
    ...
    while (majlisFiles.hasNext()) {
      ...
      try {
        ...
        // Column indexes (100 version - 8 months)
        const colMajlis = 1;    // B
        const colTanziem = 2;   // C
        const colBudget = 5;    // F
        const colBezahlt = 14;  // O (100 version)
        const colRest = 15;     // P (100 version)
        ...
        for (let tanziem of uniqueTanziem) {
          ...
          // Sum months (columns 6-13: Jul-Nov, Dec, Jan, Feb, Mar, Apr, May, Jun - 8 months)
          const sumMonths = [];
    ...
    if (collectedData.length > 0) {
      ...
      // Format Budget and months as numbers (8 months + Budget + Bezahlt + Rest = 11 columns)
      dataSheet.getRange(2, 5, collectedData.length, 11).setNumberFormat("#,##0.00");
      
      // Format Prozent as percentage
      dataSheet.getRange(2, 16, collectedData.length, 1).setNumberFormat("0.00%");
      
      dataSheet.autoResizeColumns(1, 16);
