...
// Sender Name
const SENDER_NAME = "100 Admin";

// 100 Fiscal Year: July to June
const FISCAL_START_MONTH = 7; // July

// ========================================
// CALCULATE FISCAL YEAR (Jul-Jun)
// ========================================
...
function buildMajlisEmailHTML(majlisName, khuddamData, atfalData, fiscalYear) {
  // 100 Column indexes: Anzahl=3, Nicht-Zahler=4, Budget=5, Bezahlt=14, Rest=15, Prozent=16
  ...
  const kBezahlt = khuddamData ? khuddamData[14] : 0;
  const kRest = khuddamData ? khuddamData[15] : 0;
  const kProzent = khuddamData ? (khuddamData[16] * 100).toFixed(0) : 0;
  
  // Atfal data
  const aAnzahl = atfalData ? atfalData[3] : 0;
  const aNichtZahler = atfalData ? atfalData[4] : 0;
  const aBudget = atfalData ? atfalData[5] : 0;
  const aBezahlt = atfalData ? atfalData[14] : 0;
  const aRest = atfalData ? atfalData[15] : 0;
  const aProzent = atfalData ? (atfalData[16] * 100).toFixed(0) : 0;
...
<head>
  <style>
    ...
    .data-table th { background-color: #FF6D00; color: white; font-weight: bold; }
    .title-row { background-color: #FFE0B2; font-weight: bold; }
  </style>
</head>
<body>
...
function buildRegionEmailHTML(regionName, majlisMap, fiscalYear) {
  ...
  for (let majlis in majlisMap) {
    ...
    // 100: Anzahl=3, Nicht-Zahler=4, Budget=5, Bezahlt=14, Rest=15, Prozent=16
    const kAnzahl = kData ? kData[3] : 0;
    const kNichtZahler = kData ? kData[4] : 0;
    const kBudget = kData ? kData[5] : 0;
    const kBezahlt = kData ? kData[14] : 0;
    const kRest = kData ? kData[15] : 0;
    const kProzent = kData ? (kData[16] * 100).toFixed(0) : 0;
    
    const aAnzahl = aData ? aData[3] : 0;
    const aNichtZahler = aData ? aData[4] : 0;
    const aBudget = aData ? aData[5] : 0;
    const aBezahlt = aData ? aData[14] : 0;
    const aRest = aData ? aData[15] : 0;
    const aProzent = aData ? (aData[16] * 100).toFixed(0) : 0;
